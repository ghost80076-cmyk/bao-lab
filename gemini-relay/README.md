...
5. 最後更新 Worker 程式，確認 `/health` 的 `gemini_relay_ready:true`，從不同網路來源各測一筆短句，核對模型呼叫與回覆。成功後刪除 Worker 裡舊的 `GEMINI_API_KEY`。
...
本地執行 `npm test`。部署後若還有 Google 400/403，依 Google 的安全錯誤代碼檢查帳號、金鑰與模型設定；中繼只負責隔離玩家 HTTP 標頭，不能保證供應商一定接受每次請求。
