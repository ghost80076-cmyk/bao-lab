(() => {
  ...
  const mount = (root,slots) => {
    ...
    area.querySelector("[data-main-model]")?.addEventListener("click",()=>{area.querySelector("[data-refine-model]").value=App.config?.api?.model||"";});
    area.querySelector("[data-run-refine]")?.addEventListener("click",async()=>{
      ...
      run.disabled=true;status.textContent=`整理中 · ${model}`;
      try{
    ...
    area.querySelector("[data-write-draft]")?.addEventListener("click",()=>{
      ...
      window.BAOMemoryWorkbench.writeSlots(current);App.config.memory=App.config.memory||{};App.config.memory.summaryModel=area.querySelector("[data-refine-model]").value.trim()||"";const input=document.getElementById("memory-summary-model");if(input)input.value=App.config.memory.summaryModel;App.saveStory?.(false);window.BAOMemoryWorkbench.refresh(root,"notes");
    });
  };
