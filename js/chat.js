const Chat = {
  ...
  pressure(config) {
    const limit = Math.max(1, Number(config?.memory?.maxContext || 64000));
    return this.lastStoryPromptTokens > 0 ? this.lastStoryPromptTokens / limit : 0;
  },
  ...
  protectedRounds(config) {
    ...
    if (ratio >= 0.92) {
      rounds = Math.max(4, Math.floor(configured * 0.25));
      level = "critical";
    } else if (ratio >= 0.82) {
      rounds = Math.max(6, Math.floor(configured * 0.45));
      level = "high";
    } else if (ratio >= 0.70) {
      rounds = Math.max(8, Math.floor(configured * 0.7));
      level = "watch";
    }
  ...
  async maybeSummarize(config, force = false, recentRounds = null) {
    ...
    if (force && end <= this.summarizedUntil && this.messages.length > 12) {
      end = Math.max(this.summarizedUntil, this.messages.length - Math.max(8, keepMessages));
    }
    ...
    try {
      ...
      const prompt = [
        ...
        `【待整理舊對話】\n${transcript}`,
        force
          ? "目前 Context 使用率偏高，請進一步壓縮，輸出新的完整摘要，盡量控制在 600～1200 字。"
          : "請輸出新的完整長期記憶摘要，建議 800～1600 字以內。"
      ].filter(Boolean).join("\n\n");

      const summaryConfig = { ...config.api, __memoryTask: true };
      const result = await API.send(summaryConfig, [
      ...
      if (result?.text) {
        ...
        if (window.GameState?.current) GameState.current.memory = [this.summary];
      }
    } catch (err) {
  ...
  recordStoryUsage(usage = {}, config = null) {
    this.lastStoryPromptTokens = Number(usage.prompt_tokens || 0);
    if (config) this.protectedRounds(config);
  },
  ...
  renderGuard() {
    const el = document.getElementById("usage-guard");
    if (!el) return;
    const { level, ratio, recentRounds } = this.contextGuard;
    const pct = ratio > 0 ? `${(ratio * 100).toFixed(0)}%` : "—";
    const labels = {
      normal: `正常 · 保留 ${recentRounds || "—"} 輪`,
      watch: `注意 ${pct} · 自動縮短至 ${recentRounds} 輪`,
      high: `保護中 ${pct} · 摘要＋保留 ${recentRounds} 輪`,
      critical: `緊急保護 ${pct} · 強制摘要＋保留 ${recentRounds} 輪`,
      manual: `完整上下文 · 不自動裁切`
    };
    el.textContent = labels[level] || labels.normal;
...
window.addEventListener("DOMContentLoaded", () => {
  setTimeout(() => {
    ...
    if (bar && !document.getElementById("usage-total")) {
      bar.insertAdjacentHTML("beforeend", '<span>累積 <b id="usage-total">0 tok</b></span><span>輸入累積 <b id="usage-input-total">0 tok</b></span><span>輸出累積 <b id="usage-output-total">0 tok</b></span><span>Cache 累積 <b id="usage-cache-total">0 tok</b></span><span>Context Guard <b id="usage-guard">正常</b></span>');
    }
    ...
    if (window.API && !API.__baoUsageWrapped) {
      ...
      API.send = async function(config, messages) {
        ...
        if (!config?.__memoryTask) Chat.recordStoryUsage(result?.usage || {}, App?.config);
        return result;
      };
    ...
    if (apiStep && !document.getElementById("test-api")) {
      ...
      document.getElementById("test-api").addEventListener("click", async () => {
        ...
        try {
          ...
          const previousPrompt = Chat.lastStoryPromptTokens;
          const result = await API.test({ ...config.api, __memoryTask: true });
          Chat.usage = before;
