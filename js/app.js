const App = {
  ...
  async init() {
    this.bindNavigation();
    this.bindBuilder();
    await Promise.all([this.loadCharacters(), this.loadPrompts(), this.loadModels()]);
    this.populateAPIControls();
    this.showView("home");
  },

  bindNavigation() {
    document.querySelectorAll("[data-view]").forEach(btn => btn.addEventListener("click", () => this.showView(btn.dataset.view)));
  ...
  bindBuilder() {
    document.querySelectorAll('input[name="narrative-mode"]').forEach(r => r.addEventListener("change", () => {
    ...
    document.querySelectorAll(".step").forEach(step => step.addEventListener("click", () => this.setStep(Number(step.dataset.step))));
    document.querySelectorAll(".ui-tab").forEach(tab => tab.addEventListener("click", () => {
  ...
  async loadCharacters() {
    ...
    this.characters = await Promise.all(manifest.map(async item => await (await fetch(item.file)).json()));
  ...
  async loadModels() { this.modelPresets = await (await fetch("data/presets/models.json")).json(); },
  ...
  populateAPIControls() {
    ...
    apiType.innerHTML = providers.map(provider => {
      ...
      return `<option value="${this.escapeAttr(provider)}">${this.escapeHTML(item?.provider_label || provider)}</option>`;
    }).join("");
  ...
  populateModelOptions() {
    ...
    const indexes = this.modelPresets.map((item, index) => ({ item, index })).filter(x => x.item.provider === type);
    modelSelect.innerHTML = indexes.map(({ item, index }) => `<option value="${index}">${this.escapeHTML(item.label)}</option>`).join("");
  ...
  syncSelectedPreset() {
    ...
    if (badge) badge.textContent = (preset?.protocol || "openai").toUpperCase();
    if (hint) hint.textContent = preset?.model
      ? "官方預設已填好 Model ID 與 Base URL，你只需要貼上自己的 API Key。"
      : "中轉 / 自訂：請依服務商文件填入 Model ID、Base URL 與 API Key。";
  },
  ...
  renderCharacters(filter = "all") {
    ...
    else if (filter !== "all") chars = chars.filter(c => (c.audience || []).includes(filter) || (c.categories || []).includes(filter));
    list.innerHTML = chars.length ? chars.map(c => `
      ...
      </article>`).join("") : `<p class="note">這個分類目前還沒有作品。</p>`;
  },

  openCharacter(id) { this.activeCharacter = this.characters.find(c => c.id === id); this.renderDetail(); this.showView("detail"); },
  ...
  openBuilder() { document.getElementById("builder-character-chip").textContent = this.activeCharacter.name; this.setStep(1); this.showView("builder"); },
  showCharacterDetail() { this.showView("detail"); },

  setStep(step) {
    this.currentStep = Math.max(1, Math.min(5, step));
    document.querySelectorAll(".builder-step").forEach(p => p.classList.toggle("active", Number(p.dataset.stepPanel) === this.currentStep));
    document.querySelectorAll(".step").forEach(s => s.classList.toggle("active", Number(s.dataset.step) === this.currentStep));
  ...
  startStory() {
    ...
    if (!this.config.api.model) { alert("請先輸入 Model ID。"); this.setStep(4); return; }
    if (!this.config.api.baseUrl) { alert("請先填入 Base URL。"); this.setStep(4); return; }
    if (!this.config.api.key) { alert("請先填入 API Key。"); this.setStep(4); return; }
    ...
    document.getElementById("usage-memory").textContent = this.config.memory.mode === "smart" ? "0 輪 · 智慧記憶" : `0/${this.config.memory.maxRounds}`;
    ...
    document.getElementById("chat-character-card").innerHTML = `<img src="${this.escapeAttr(this.activeCharacter.avatar)}" alt="${this.escapeAttr(this.activeCharacter.name)}"><div class="eyebrow">ACTIVE CHARACTER</div><h3>${this.escapeHTML(this.activeCharacter.name)}</h3>`;
    document.getElementById("chat-stream").innerHTML = `<div class="message assistant"><div class="bubble">${this.formatMessage(this.activeCharacter.greeting)}</div></div>`;
  ...
  renderUIPanel(panel) {
    ...
    if (panel === "npc") {
      ui.innerHTML = `<div class="npc-grid">${(s.npcs || []).map(n => `<div class="npc-card"><strong>${this.escapeHTML(n.name)}</strong><br><span>${this.escapeHTML(n.role || "NPC")}</span><br><span>情緒：${this.escapeHTML(n.mood || "未知")}</span><br><span>位置：${this.escapeHTML(n.location || "未知")}</span><br><span>關係：${this.escapeHTML(String(n.relationship ?? "未設定"))}</span></div>`).join("") || "目前沒有 NPC。"}</div>`;
    } else if (panel === "status") {
      ui.innerHTML = `<div class="state-grid"><div class="state-box"><small>時間</small><b>${this.escapeHTML(s.time)}</b></div><div class="state-box"><small>地點</small><b>${this.escapeHTML(s.location)}</b></div><div class="state-box"><small>敘事</small><b>${this.config.narrativeMode === "world" ? "世界模擬" : "單角色沉浸"}</b></div></div>`;
    } else if (panel === "events") {
      ui.innerHTML = (s.events || []).map(x => `• ${this.escapeHTML(x)}`).join("<br>");
    } else if (panel === "memory") {
      if (this.config.memory.mode === "smart") {
        ui.innerHTML = Chat.summary
          ? `<b>長期記憶摘要</b><div style="margin-top:8px">${this.formatMessage(Chat.summary)}</div><p class="note">較新的 ${this.config.memory.maxRounds} 輪仍保留原文；更早內容會持續壓縮進這份摘要。</p>`
          : `<span class="note">智慧記憶已啟用。對話累積超過保留範圍後，舊內容會自動整理成長期摘要。</span>`;
      } else {
        ui.innerHTML = s.memory.length ? s.memory.map(x => `• ${this.escapeHTML(x)}`).join("<br>") : `目前尚無長期記憶。最近保留 ${this.config.memory.maxRounds} 輪。`;
      }
    }
  },
  ...
  buildSystemPrompt() {
    ...
    return [c.system_prompt, "", "【敘事模式】", mode.prompt, "", "【玩家 Persona】", `名稱：${p.name}`, `性別：${p.gender}`, `身分：${p.identity || "未指定"}`, `個性：${p.personality || "未指定"}`, `與角色的初始關係：${p.relationship || "未指定"}`, `其他設定：${p.extra || "無"}`, "", "不得替玩家決定台詞、心理或行動。", this.config.displayMode === "ui" ? "目前使用互動 UI。不要每輪重新輸出完整 UI HTML，敘事正常輸出即可。" : "目前使用純文本模式。不要輸出 RPG 狀態面板。"].join("\n");
  },
  ...
  async sendMessage() {
    ...
    stream.insertAdjacentHTML("beforeend", `<div class="message user"><div class="bubble">${this.formatMessage(text)}</div></div>`);
    input.value = "";
    Chat.add("user", text);
    ...
    try {
      const memoryContext = await Chat.context(this.config);
      const messages = [{ role: "system", content: this.buildSystemPrompt() }, ...memoryContext];
      ...
      document.getElementById("usage-context").textContent = usage.prompt_tokens ? `${usage.prompt_tokens.toLocaleString()} tok` : "—";
      document.getElementById("usage-turn").textContent = usage.completion_tokens ? `${usage.completion_tokens.toLocaleString()} tok` : "—";
      document.getElementById("usage-cache").textContent = usage.cached_tokens ? `${usage.cached_tokens.toLocaleString()} tok` : "—";
      document.getElementById("usage-memory").textContent = Chat.memoryStatus(this.config.memory.maxRounds);
      if (this.config.displayMode === "ui" && document.querySelector('.ui-tab[data-panel="memory"]')?.classList.contains("active")) this.renderUIPanel("memory");
      stream.scrollTop = stream.scrollHeight;
    } catch (err) {
  ...
  exitChat() { this.showView("detail"); },
  showView(name) { document.querySelectorAll(".view").forEach(v => v.classList.remove("active")); document.getElementById(`${name}-view`)?.classList.add("active"); window.scrollTo({ top: 0, behavior: "instant" }); },
  formatMessage(str = "") { return this.escapeHTML(str).replace(/\n/g, "<br>"); },
