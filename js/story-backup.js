(() => {
  ...
  const sanitizeCheckpoint = checkpoint => {
    ...
    return { messageId, seq, updatedAt: String(checkpoint.updatedAt || new Date().toISOString()), payload };
  };
  ...
  const sanitizeBundle = input => {
    ...
    const chapters = input.chapters.map((entry, index) => {
      ...
      return {
        ...
        payload, checkpoints: (Array.isArray(entry.checkpoints) ? entry.checkpoints : []).map(sanitizeCheckpoint).filter(Boolean)
      };
    });
    ...
    for (const chapter of chapters) {
      ...
      let parent = chapter.parentChapterId;
      while (parent) {
        if (seen.has(parent)) throw new Error('完整故事備份包含循環的分支關係。');
        seen.add(parent);
        parent = chapters.find(item => item.chapterId === parent)?.parentChapterId || '';
      }
    }
    return {
      schema: SCHEMA, version: Math.max(1, Number(input.version || VERSION)),
      exportedAt: String(input.exportedAt || new Date().toISOString()), story,
      activeChapterId: String(input.activeChapterId || story.activeChapterId || chapters[0].chapterId), chapters
    };
  };
  ...
  const buildBundle = async (storyId = Library.refs().storyId) => {
    ...
    const chapterRecords = records.filter(record => record.kind === 'chapter' && record.storyId === targetStoryId)
    ...
    for (const chapter of chapterRecords) {
      ...
      const checkpoints = records.filter(record => record.kind === 'checkpoint' && record.storyId === targetStoryId && record.chapterId === chapter.chapterId && record.payload)
        .sort((a, b) => Number(a.seq || 0) - Number(b.seq || 0)).map(sanitizeCheckpoint).filter(Boolean);
      chapters.push({
        chapterId: chapter.chapterId, parentChapterId: chapter.parentChapterId || '',
    ...
    const activeChapterId = refs.storyId === targetStoryId && refs.chapterId ? refs.chapterId : (storyRecord.activeChapterId || chapters[0].chapterId);
    return scrub({
      schema: SCHEMA, version: VERSION, exportedAt: new Date().toISOString(),
      story: {
        storyId: targetStoryId, title: storyRecord.title || storyRecord.characterName || '未命名故事',
        ...
        updatedAt: storyRecord.updatedAt || chapters.at(-1)?.updatedAt || new Date().toISOString(), activeChapterId
  ...
  const importBundle = async input => {
    ...
    const chapterMap = {};
    bundle.chapters.forEach(chapter => { chapterMap[chapter.chapterId] = Library.id(chapter.parentChapterId ? 'branch' : 'chapter'); });
    ...
    try {
      for (const chapter of bundle.chapters) {
        ...
        payload._library = {
          storyId, chapterId, storyCreatedAt, chapterCreatedAt: chapter.createdAt || payload.savedAt,
          chapterLabel: chapter.label, parentChapterId, branchPointMessageId: chapter.branchPointMessageId || '',
          branchPointSeq: Number(chapter.branchPointSeq ?? -1), branchPointPreview: chapter.branchPointPreview || ''
        };
        ...
        const importedCheckpoints = chapter.checkpoints.filter(checkpoint => messageIds.has(checkpoint.messageId)).map(checkpoint => {
          ...
          return {
            ...
            kind: 'checkpoint', storyId, chapterId, messageId: checkpoint.messageId,
            seq: checkpoint.seq, updatedAt: checkpoint.updatedAt || payload.savedAt, payload: checkpointPayload
          };
        });
        if (importedCheckpoints.length) await Library.transaction('readwrite', store => importedCheckpoints.forEach(record => store.put(record)));
      }
      const desiredOldChapterId = bundle.activeChapterId && chapterMap[bundle.activeChapterId] ? bundle.activeChapterId : bundle.chapters[0].chapterId;
      ...
      return {
        storyId, chapterId: activeChapterId, chapterCount: bundle.chapters.length,
        title: String(bundle.story.title || importedActive.characterName || '匯入故事'),
        chapterMap: clone(chapterMap), payload: importedActive
      };
    } catch (error) {
      try { await Library.deleteStory(storyId); } catch (cleanupError) {
        throw new Error('匯入失敗，且無法清除部分匯入的故事；請檢查 Story Library。', { cause: error });
      }
      throw error;
    } finally {
  ...
  const exportCurrentStory = async () => { const bundle = await buildBundle(); downloadBundle(bundle); return bundle; };
  ...
  const installExportButton = () => {
    ...
    button.id = 'export-story-button'; button.className = 'secondary'; button.type = 'button'; button.textContent = '完整故事備份';
  ...
  window.BAOStoryBackup = { schema: SCHEMA, version: VERSION, isBundle, sanitizeBundle, buildBundle, importBundle, downloadBundle, exportCurrentStory };
  installExportButton();
})();
