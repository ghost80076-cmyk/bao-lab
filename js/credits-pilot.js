...
(() => {
  ...
  // The model catalog loads asynchronously. Install before its first render and also
  // handle a page where the catalog already finished loading.
  ...
  const refreshBuilder = () => {
    ...
    if (enabled && hint) hint.textContent = "邀請制測試：請填個人玩家金鑰，不是管理員密鑰。故事內容將經 BAO/LAB 後端轉送 Google；目前僅限文字、非串流，每次最多 24 KB，請勿傳送私人資訊。玩家金鑰不會存入故事備份。";
  ...
  API.send = async function(config, messages) {
    // Helpers normally inherit the main key. Never forward a player token to a
    // different provider if a helper route was accidentally left without its key.
    if (!isPilot(config)) {
    ...
    if (!Array.isArray(messages) || !messages.length || messages.length > 40) {
      throw new Error("BAO/LAB 測試版每次最多傳送 40 則訊息。請縮短近期對話，或改用自己的 API Key。");
    }
    const cleaned = messages.map(m => ({ role: m.role, content: this.contentToText(m.content) }));
    if (!cleaned.some(m => m.role === "user") || cleaned.some(m => !["user", "assistant", "system"].includes(m.role) || !m.content || m.content.length > 16000) ||
        encoder.encode(JSON.stringify(cleaned)).length > 24000) {
    ...
    if (!response.ok) {
      const errors = {
        ...
        daily_limit_or_insufficient_balance: "今日測試次數已達上限，或測試額度不足。",
        insufficient_balance: "玩家測試額度不足。",
        invalid_request_or_model_not_allowed: "模型未開放或故事內容不符合測試版限制。",
        provider_request_failed: "Gemini 暫時無法回覆，可能是免費 API 額度、模型限制或服務異常。",
        origin_not_allowed: "目前網站網址不在 Worker 的允許來源清單。"
      };
      throw new Error(errors[data?.error] || `BAO/LAB 測試後端錯誤（HTTP ${response.status}）。`);
    }
    ...
    return {
      ...
      credits: { charged_microusd: data.usage?.charged_microusd, request_id: data.request_id }
    };
  };
  ...
  const refreshDialog = backdrop => {
    ...
    if (pilot && hint) hint.textContent = "邀請制測試：此處填玩家金鑰，不是 ADMIN_TOKEN。故事內容會經 BAO/LAB 後端轉送 Google。金鑰不會寫入故事備份；僅支援文字、非串流。";
  };
  const watchDialog = () => {
  ...
  const init = () => {
    ensurePresets();
    // A late-loaded module must not reset the model and key fields the user has entered.
    refreshBuilder();
