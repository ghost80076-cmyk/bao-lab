(() => {
  ...
  App.collectConfig=function(){const cfg=originalCollect();if(this.activeCharacter?.id!==ID)return cfg;const section=document.getElementById('autonomous-world-setup');if(!section)return cfg;for(const key of ['mode','world','references','tone','worldText','npcs','personaText','opening'])draft[key]=text(key);saveDraft();cfg.worldSetup=JSON.parse(JSON.stringify(draft));cfg.narrativeMode=draft.mode==='immersive'?'immersive':'world';return cfg;};
