...
先部署 [台灣區 Gemini 中繼服務](../gemini-relay/README.md)。在 Cloudflare Worker 的 Secrets 設定 `GEMINI_RELAY_URL`（Cloud Run HTTPS 基底網址，結尾 `/`）及 `GEMINI_RELAY_TOKEN`（與中繼服務一致的隨機權杖）。中繼服務單獨保管 `GEMINI_API_KEY`；Worker 舊有的 `GEMINI_API_KEY` 不再使用，確認新路徑上線後從 Worker 刪除。**切勿先覆蓋正在服務的 Worker 再部署中繼服務**：缺中繼設定時 Gemini 請求會安全失敗。此中繼設計的目的是不把玩家來源 IP 標頭轉給 Gemini；它不依玩家所在地切換後端或拒絕請求。

在 Cloudflare → Workers & Pages → `bao-lab-credits-api` → Edit code，把 [最新 index.js](./src/index.js) 的**完整內容**覆蓋原有 Worker 程式，按 Deploy。GitHub 原始碼**不是已部署的正式服務**。不用重建 D1；`ADMIN_TOKEN`、`ALLOWED_ORIGIN` 沿用原值。確認 `https://bao-lab-credits-api.ghost80076.workers.dev/health` 顯示 `"diagnostic_version":"2026-09-22-relay-3"`、`"gemini_relay_ready":true`。這只是設定檢查；最後需從不同來源各實測一筆短句，核對中繼與 Google 的實際回應。
