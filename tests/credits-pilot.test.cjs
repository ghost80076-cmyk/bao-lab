...
function build(response={ok:true,status:200,body:{content:'測試成功',usage:{input_tokens:10,output_tokens:6,charged_microusd:0}}}) {
...
test('pilot sends player-token request to fixed Worker, never admin/provider keys',async()=>{
 ...
 assert.deepEqual(JSON.parse(JSON.stringify(JSON.parse(opt.body))),{provider:'gemini',model:'gemini-3-flash-preview',request_kind:'chat',messages:msgs,max_output_tokens:2048});
 assert.equal(opt.body.includes(token),false);
});
test('summary routes separate from chat and BYOK stays available',async()=>{
 ...
 assert.equal(normal.text,'BYOK works'); assert.equal(s.calls.length,1);
});
...
test('limits excessive prompts and propagates backend errors without secrets',async()=>{
 const s=build({ok:false,status:401,body:{error:'unauthorized'}});
 await assert.rejects(()=>s.api.send(cfg,msgs),/玩家金鑰無效/);
 assert.equal(s.calls.length,1);
 await assert.rejects(()=>s.api.send(cfg,[{role:'user',content:'x'.repeat(25000)}]),/24 KB/);
 assert.equal(s.calls.length,1);
});
