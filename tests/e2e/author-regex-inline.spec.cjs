...
const rules = { regex_scripts: [
  ...
  { scriptName: '介面按鈕', findRegex: '【開屏1】', replaceString: '<div class="author-panel"><button onclick="BAOAuthor.draft(\'查看照片\')">查看照片</button></div>' }
] };
...
test('inline author scripts remain disabled by default', async ({ page }) => {
  await startDemo(page);
  await page.evaluate(() => { Chat.add('assistant', '【開屏1】'); App.renderChatShell(false); });
  const panel = page.locator('#bao-author-regex-panel');
  await panel.locator('summary').click();
  await panel.locator('input[type=file]').setInputFiles({ name: 'author-regex.json', mimeType: 'application/json', buffer: Buffer.from(JSON.stringify(rules)) });
  await panel.getByLabel('在這張角色卡啟用作者介面').check();
  await expect(page.locator('#chat-stream .bao-author-inline')).toHaveCount(0);
  await expect(page.locator('#chat-stream .message.assistant').last().locator(':scope > .bubble')).toContainText('【開屏1】');
});
